...
####################### send packet one at a time and wait for ACK response ###############################
    ...
    # packet_list, checksum = make_packets(data, SEQ)
        ...
        while isACK == False:
            ...
            #intercept ACK recieve to test against loss
            while timed_out == True:
                udp_client.sendto(packet, (host, port))

                # Try except to handle full timeout from socket
                try:
                    rcv_packet = udp_client.recv(packet_size)
                    if int(corrupt_option) == 4 and random.randint(0, 100) < int(error_chance):
                        pass
    ...
    while True:    
        ...
        match option:
            ...
            case '3':
                ...
                break
            case '4':
                print('Option 4 - ACK packet loss')
                error_chance = input('Enter percentage of loss rate between 0-100: ')
                break
            case '5':
                print('Option 5 - Data packet loss')
                error_chance = input('Enter percentage of loss rate between 0-100: ')
                break
            case _:
